...
WITH
  ...
  SELECT 
    ...
    nnet_score
  FROM 
    `emlab-gcp.emlab_test.pipe_v20201001_fishing_subset2020`
  WHERE 
  # Keep only data for January 2020 
  ...
  fishing_binned AS (
  SELECT
    # Convert lat/lon to 0.1 degree bins and numeric values
...
fishing_map <- ggplot() +
  ...
  theme(panel.border = element_blank(), 
        ...
        legend.text = element_text(color = "#848b9b", size = 12), 
        legend.title = element_text(color = "#363c4c", size = 12), 
        plot.title = element_text(color = "#363c4c", size = 14), 
        plot.subtitle = element_text(color = "#363c4c", size = 14), 
        axis.title = element_blank(), 
        axis.text = element_text(size = 6)) 
```

```{r display-map, fig.width=6, fig.height=4, message=F, warning=F, echo=F}
fishing_map 
```
